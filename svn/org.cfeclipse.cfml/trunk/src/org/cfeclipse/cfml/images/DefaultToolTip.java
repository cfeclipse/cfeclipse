...
public class DefaultToolTip extends ToolTip {
	...
	protected Color getForegroundColor(Event event) {
		return (foregroundColor == null) ? event.widget.getDisplay().getSystemColor(SWT.COLOR_INFO_FOREGROUND)
				: foregroundColor;
	}
	...
	protected Color getBackgroundColor(Event event) {
		return (backgroundColor == null) ? event.widget.getDisplay().getSystemColor(SWT.COLOR_INFO_BACKGROUND)
				: backgroundColor;
	}
	 ...
	 * @param event
	 *            the event triggered the popup of the tooltip
	 * @return the {@link Font} or <code>null</code> if the default font should
	 *         be used
	 */
